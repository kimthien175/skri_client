...
class PlayerInfoDialog extends GameDialog {
  ...
  static Future<void> delete(String playerId) =>
      OverlayController.deleteCache('card_info_$playerId');
    ...
    return Row(
      ...
      children: [
        ...
        (info.id == MePlayer.inst.id)
            ...
            : Column(
                children: [
                  if (gameInst is PrivateGame &&
                      (gameInst.hostPlayerId.value == MePlayer.inst.id)) ...[
                    const Row(
                      children: [
                        // KICK
                        _KickButton(),
                          ...
                          onTap: () {
                            if (infoDialog.isShowing) {
                              ...
                              ReportFormDialog.factory(
                                info,
                              ).showInstantly().then((value) => info.isReported = value);
                            }
                          },
    ...
    return Column(
      children: [
        ...
        HoverButton(
          ...
          height: 34.5,
          constraints: _InfoDialogContent.inputConstraints,
          child: Text('click_to_copy_invite'.tr, style: const TextStyle(fontSize: 15)),
        ),
      ],
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return HoverButton(
      ...
      constraints: const BoxConstraints(
        minWidth: _InfoDialogContent.minWidth / 2 - 3,
        minHeight: _InfoDialogContent.minHeight,
      ),
      onTap: () {
        if (isDisabled) return;
        pauseFncBeforeSending();
