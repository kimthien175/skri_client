...
class ReportFormDialog extends GameDialog {
  ReportFormDialog._internal({required this.info})
    : super(
        title: Builder(builder: (context) => Text(info.name)),
        content: const _ReportContent(),
        buttons: RowRenderObjectWidget(
          children: [
            GameDialogButton(
              onTap: (onQuit) async {
                var dialog =
                ...
                if (res.statusCode == 200) {
                  dialog.content.value = Center(
                    child: Text('Your report has been received, thank you'),
                  );
                  dialog.buttons.value = null;
  ...
  factory ReportFormDialog.factory(Player info) {
    return OverlayController.put(
      tag: 'report ${info.id}',
      builder: () => ReportFormDialog._internal(info: info),
    );
  }
  bool reason1 = false;
  bool reason2 = false;
...
class _ReportContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ReportContentObjectWidget(
      children: [
