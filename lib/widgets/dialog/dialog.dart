...
class GameDialog extends OverlayController with GetSingleTickerProviderStateMixin {
  ...
  GameDialog.error({
    required Widget content,
    RowRenderObjectWidget buttons = const RowRenderObjectWidget(
      children: [GameDialogButton.okay()],
    ),
    this.onQuit = GameDialog.onQuitDefault,
      ...
      children: [
        Container(
          ...
          child: Obx(() {
            ...
            return _DialogRenderObjectWidget(
              children: [
                const _Title(),
...
class _CloseButtonState extends State<_CloseButton> with SingleTickerProviderStateMixin {
  ...
  late final Animation<Color?> colorAnim = ColorTween(
    begin: const Color.fromRGBO(170, 170, 170, 1),
    end: Colors.white,
  ).animate(controller);
