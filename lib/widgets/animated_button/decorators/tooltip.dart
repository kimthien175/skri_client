...
class AnimatedButtonTooltipDecorator implements AnimatedButtonDecorator {
  AnimatedButtonTooltipDecorator(
      {required this.childBuilder, this.position = const GameTooltipPosition.centerTop()});
  ...
  void decorate(AnimatedButtonState state) {
    _tooltip = _Tooltip(childBuilder: childBuilder, position: position, state: state);
...
class _Tooltip extends GameTooltip {
  _Tooltip({required super.childBuilder, required this.state, super.position})
